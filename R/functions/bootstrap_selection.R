#' Bootstrap selection coefficients
#'
#' Calculates percentile confidence intervals for selection coefficients using non-parametric bootstrapping.
#'
...
#' @param B Integer specifying the number of bootstrap replicates. Default is 1000.
#' @param conf Numeric value for the confidence interval level. Default is 0.95.
#' @param seed Optional integer to set the random seed for reproducibility.
#'
#' @return A list containing \code{draws} (all bootstrap estimates), \code{ci} (confidence intervals), and \code{success_rate}.
#' @export
#'
#' @examples
#' \dontrun{
#' boot_res <- bootstrap_selection(my_data, "survival", c("size", "color"))
#' }
